...
import numpy as np
from networks.HybridMamba.bi_vision_mamba import Mamba
from monai.networks.blocks.unetr_block import UnetrUpBlock
from networks.HybridMamba.semantic_difference_module import SDN
...
class MaskedAutoencoderMamba(nn.Module):
    ...
    def __init__(self, img_size=96, downsample_rato=16, embed_dim=384, depth=8, norm_layer=nn.LayerNorm):
        super().__init__()
        # --------------------------------------------------------------------------
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.SDN4 = SDN(channels[4],channels[4])
        self.SDN3 = SDN(channels[3],channels[4])
#        self.SDN2 = SDN(channels[2],channels[3])
#        self.SDN1 = SDN(channels[1],channels[2])
        self.up = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=True)
        self.out = UnetOutBlock(in_channels=channels[0], n_classes=n_classes)

    def forward(self, x1, x2, x3, x4, x5):
        x4 = self.SDN4(x4,self.up(x5))
        d4 = self.decoder5(x5, x4)
        x3 = self.SDN3(x3,self.up(d4))
        d3 = self.decoder4(d4, x3)
#        x2 = self.SDN2(x2,self.up(d3))
        d2 = self.decoder3(d3, x2)
#        x1 = self.SDN1(x1,self.up(d2))
